...
namespace lmi {
    ...
    std::unique_ptr<LMIFunction> makeOAIRequest(const std::string& apiKey, const std::string& model, const std::string& prompt, int maxValidResponses=1, int maxRetries=3) {
        ...
        while (validResponses.size() == 0 || (validResponses.size() > maxValidResponses) && attempt < maxRetries) {
            ...
            auto tryCreate = [&rawJson, &instances](auto dummy) {
